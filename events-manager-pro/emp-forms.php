	...
	function get_post( $validate = true ) {
		...
		foreach($this->form_fields as $field) {
			...
			if ( 'checkboxes' == $field['type'] ) {
				...
				// two options:
				// - either 'options_selection_values' is a valid JSON string
				// - or can be exploded by breakybreakies.
				// hiero
				$checked_options = [];

				if ( json_validator( $field['options_selection_values'] ) ) {
				
					$values = json_decode( $field['options_selection_values'] );
					foreach ( $values as $tempjson_object ) { 
						
						$counter++; // this counter makes the input unique
						$checkboxvalue  	= sanitize_title( $value ); // the value may be anything, even HTML. This would be unacceptable for the value attribute;
						$label				= $value;
						$valueoud			= $value;
						$extrahtml			= '';
						$checkboxid			= '';
					
						$checkboxid     	= sanitize_title( $field['fieldid'] . $counter ); // the value may be anything, even HTML. This would be unacceptable for the value attribute;
						
						if ( isset( $_REQUEST[ $checkboxid ] ) ) { 
							// a value has been submitted for this key

							if ( $tempjson_object->value ) {
								$checked_options[] = esc_html( $tempjson_object->value );
							}
							else {
								$checked_options[] = esc_html( $_REQUEST[ $checkboxid ] );
							}
						}
					}	
				}
					...
					$possible_options	= explode("\r\n",$field['options_selection_values']);
					$checked_options	= array(); // empty array
					
					foreach ( $possible_options as $possiblekey => $possiblevalue ) {
						// go through all possible options and see if one of them is checked
						$possible_options[ $possiblekey ] = html_entity_decode( trim( $possiblevalue ) );
						
						if ( trim( $possiblevalue ) ) {
							// this avoids any possible empty values
							$counter++; // this counter makes the input unique
							$checkboxid     = sanitize_title( $field['fieldid'] . $counter );
							
							if ( isset( $_REQUEST[ $checkboxid ] ) ) { 
								// a value has been submitted for this key
								$checked_options[] = $possiblevalue;
							}
						}
					}
				}
...
//					error_log( 'get_post: ' . $key . ' - ' .  $value );
//				}

				$this->field_values[$fieldid] = $checked_options;
				
			} elseif ( 'radio' == $field['type'] ) {
				$this->field_values[$fieldid]	= sanitize_title( $_REQUEST[$fieldid] );
			...
			//if this is a custom user field, change $field to the original field so the right date/time info is retreived
	...
	function output_field($field, $post=true) {
		...
		switch($field['type']) {
			...
			case 'checkboxes':
				// wrap the checkboxes in a fieldset with a legend for explaining what the group is about and add labels to each checkbox
				echo '<fieldset class="input-' . $field['type'] . ' input-field-' . $field['fieldid'] . '">';
				echo '<legend>';
				echo $field['label'] . $required;
				echo '</legend>';
				if ( !empty($field['options_'.$tip_type.'_tip']) ): 
					echo '<span class="form-tip">' . esc_attr($field['options_'.$tip_type.'_tip']) . '</span>';
				endif; 
				echo $this->output_field_input($field, $post);
				echo '</fieldset>';
				break;
        ...
        // this applies to 
        // 'text', 'textarea', 'checkbox', 'date', 'select', 'country', 'multiselect', 
		...
		switch($field['type']) {
			...
			case 'checkboxes':
				...
				if ( json_validator( $field['options_selection_values'] ) ) {
					...
					foreach ( $values as $tempjson_object ) { 
						...
						$counter++; // this counter makes the input unique
						$checkboxvalue  	= ''; // the value may be anything, even HTML. This would be unacceptable for the value attribute;
						$label				= '';
						$valueoud			= '';
						$extrahtml			= '';
						$checkboxid			= '';
						...
						if ( $tempjson_object->value ) {
							$checkboxvalue	= esc_html( $tempjson_object->value );
						}
						if ( $tempjson_object->label ) {
							$label			= wp_kses_decode_entities( $tempjson_object->label );
						}
						if ( $tempjson_object->extrahtml ) {
							$extrahtml		= wp_kses_decode_entities( $tempjson_object->extrahtml );
						}
						
						if ( is_admin() ) {
							?>
								<div style="float: none; clear: both; overflow: hidden;"><input id="<?php echo $checkboxid ?>" type="checkbox" name="<?php echo $checkboxid ?>" value="<?php echo esc_html( $checkboxvalue )?>"<?php if (in_array( $checkboxvalue, $default)) echo 'checked="checked"'; ?> style="float: left; margin: .25em;">
									<label for="<?php echo $checkboxid ?>" style="float: left;"><?php echo $label ?></label></div>
							<?php
						} else {
							?>
							<div class="separator" id="separator-<?php echo $checkboxid ?>">
								<div class="input-extras">
								<input id="<?php echo $checkboxid ?>" type="checkbox" name="<?php echo $checkboxid ?>" value="<?php echo esc_html( $checkboxvalue )?>"<?php if (in_array( $checkboxvalue, $default)) echo 'checked="checked"'; ?>>
								</div>
								<div class="label-extras">
									<label for="<?php echo $checkboxid ?>"><?php echo $label ?></label>
									<?php echo $extrahtml ?>
								</div>
							</div>
							<?php	
					...
					foreach ( $values as $value ) { 
						...
						if ( $value ) {
							...
							<div class="separator">
								<div class="input-extras">
									<input id="<?php echo $checkboxid ?>" type="checkbox" name="<?php echo $checkboxid ?>" value="<?php echo esc_html( $checkboxvalue )?>"<?php if (in_array($value, $default)) echo 'checked="checked"'; ?>>
								</div>
								<div class="label-extras">
									<label for="<?php echo $checkboxid ?>"><?php echo $label ?></label>
								</div>
			...
			case 'radio':
				...
				foreach ( $values as $value ) { 
					...
					if ( $value ) {
						$counter++; // this counter makes the input unique
						$radiobuttonid    = sanitize_title( $field['fieldid'] . $counter ); // the value may be anything, even HTML. This would be unacceptable for the value attribute;
						$radiobuttonvalue = sanitize_title( $value ); // the value may be anything, even HTML. This would be unacceptable for the value attribute;
						
						$checked	= ''; 
						if ( $default && ( $radiobuttonvalue === $default ) ) {
							$checked	= ' checked="checked"';
						}

						if ( is_admin() ) {
							?>
							<div class="separator" style="clear: both; overflow: hidden;">
								<div class="input-extras">
									<input id="<?php echo $radiobuttonid ?>" type="radio" name="<?php echo $field_name ?>" value="<?php echo $radiobuttonvalue ?>"<?php echo $checked; ?> style="float: left; margin: .25em;">
								</div>
								<div class="label-extras">
									<label for="<?php echo $radiobuttonid ?>" style="float: left;"><?php echo $value ?></label>
								</div>
							</div>
							<?php
						} else {
							...
							<div class="separator">
								<div class="input-extras">
									<input id="<?php echo $radiobuttonid ?>" type="radio" name="<?php echo $field_name ?>" value="<?php echo $radiobuttonvalue ?>"<?php echo $checked; ?>>
								</div>
								<div class="label-extras">
									<label for="<?php echo $radiobuttonid ?>"><?php echo $value ?></label>
								</div>
							</div>
						<?php
						}
...
//						$checks = '';
						// user must check one or more of the possible options
// hiero
// json						
				...
				if ( json_validator( $field['options_selection_values'] ) ) {
					...
					foreach ( $values as $tempjson_object ) { 
						...
						$checkboxid     	= sanitize_title( $field['fieldid'] . $counter ); // the value may be anything, even HTML. This would be unacceptable for the value attribute;
								...
								// $checks .= ' (poss.val: -) id="' . $checkboxid . '" is ';
						...
						foreach ( $possible_options as $possiblekey => $possiblevalue ) {
							...
							if ( trim( $possiblevalue ) ) {
								...
								if ( isset( $_REQUEST[ $checkboxid ] ) ) { 
									// a value has been submitted for this key
									$checked_options = $possible_options;
									$result = true;
						...
						if ( empty( $checked_options ) ) {
							// the array is still empty, so boohoo, validation = failed
							$result		= false;
				...
				case 'radio':
					...
					if ( ! empty( $field['required'] ) ) {
						...
						if ( trim( $_REQUEST[ $field['fieldid'] ] ) ) { 
							// a value has been submitted for this key
							$result	= true;
						} else {
							// no value submitted for this radiobutton group
							$result		= false;
							$this_err	= (!empty($field['options_selection_error'])) ? $field['options_selection_error']:$err;
							$this->add_error($this_err);
						}
							...
							<div class="bct-selection bct-options" style="display:none;">
								<div class="bct-field">
									...
									<div class="bct-input">
										...
										<!-- 
											this really needs a different structure: 
											value and labels should be separated; 
											values should NOT allow HTML within them; 
											labels should allow for basic HTML 
  ...
  "confday_monday": {
    "value": "monday",
    "label": "<span>19 Nov</span>Monday: Pre-conference day",
    "extrahtml": "<p>For members of the international community only.</p>"
  },
  "confday_tuesday": {
    "value": "tuesday",
    "label": "<span>20 Nov</span>Tuesday: Day 1",
    "extrahtml": "<p>For everyone</p>"
  },
  "confday_wednesday": {
    "value": "wednesday",
    "label": "<span>21 Nov</span>Wednesday: Day 2",
    "extrahtml": "<p>For everyone</p>"
  },
  "other option": {
    "value": "value B",
    "label": "label for second option.",
    "extrahtml": "<p>And again: any HTML here.</p><p>Please escape quotes: \" and also \'</p>"
  }
}';
										$jsonoption		= sprintf(__( 'OR:<br>Advanced use: add <span style="background: yellow; padding: .1em .25em;">valid JSON</span> for the options. Example: <pre>%s</pre> You may want to use <a href="%s">%s</a> to check your JSON. Notice these keys: <pre>value, label, extrahtml </pre>Class-attributes are not (yet) supported, sorry.', 'em-pro' ), esc_html( $example ), 'https://jsoneditoronline.org/', 'jsoneditoronline.org' );

										?>
										<textarea rows="6" name="options_selection_values[]"><?php self::input_default('options_selection_values',$field_values,'textarea'); ?></textarea>
										<?php if ( EM_ML::$is_ml ) $this->editor_ml_fields_trigger('options_selection_values'); ?>
										<em><?php _e('Available options, one per line.','em-pro'); ?></em><br><?php echo $jsonoption ?>
