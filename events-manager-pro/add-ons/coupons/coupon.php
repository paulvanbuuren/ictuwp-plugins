...
class EM_Coupon extends EM_Object {
	...
	function __construct($id = false, $search_by = 'id') {
		global $wpdb;
		//Initialize
		$this->required_fields = array("coupon_name" => emp__('Name', 'events-manager'), "coupon_discount" => __('Discount', 'em-pro'), "coupon_code" => __('Code', 'em-pro'));
	...
	function recount( $count = false ){
		...
		if( !$count ){
			...
			$count = $wpdb->get_var('SELECT COUNT(*) FROM '.EM_BOOKINGS_TABLE." WHERE booking_meta LIKE '%{$coupon_search}%'");
		}
